defmodule Mnemonex.CoderTest do
  ...
  test "encode", %{coder: coder} do
    ...
    assert Mnemonex.encode(<<1, 2, 3, 4, 5, 6, 7>>, coder) ==
             "papa-twist-alpine--chess-flute-ego\n"

    assert Mnemonex.encode(<<1, 2, 3, 4, 5, 6, 7, 8>>, coder) ==
             "papa-twist-alpine--content-sailor-athena\n"

    assert Mnemonex.encode(0x0102030405060708, coder) ==
             "papa-twist-alpine--content-sailor-athena\n"
  ...
  test "soundalike", %{coder: coder} do
    assert Mnemonex.decode("colombo-academy", coder) ==
             Mnemonex.decode("columbo-academie", coder),
           "Both included == both sound-alike"

    assert Mnemonex.decode("hullo-analog", coder) == Mnemonex.decode("hello-analogue", coder),
           "Mixed type on both sides"
  end
  ...
  test "fuzzy round trip", %{coder: coder} do
    test_em = fn
      ...
      [r | rest], fun ->
        assert r |> Mnemonex.encode(coder) |> Mnemonex.decode(coder) == r
        fun.(rest, fun)
    end
