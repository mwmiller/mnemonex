defmodule Mnemonex.State do
  ...
  defp word_map(words) do
      words
        |> Tuple.to_list
        |> Enum.zip(0 .. tuple_size(@words_tuple) - 1)
  ...
  This state remains unchanged throught the process lifetime. If attempting
  to use a custom state, care must be taken to keep all items aligned
  """
  def init do
      struct(__struct__, words: @words_tuple, word_indices: word_map(@words_tuple), words_version: @words_version,
                         total_words: 1633, base_words: 1626, rem_words: 7,
            )
  end
